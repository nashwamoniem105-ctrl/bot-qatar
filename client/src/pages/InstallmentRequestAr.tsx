...
export default function InstallmentRequestAr() {
  ...
  const validateForm = () => {
    ...
    if (!formData.ownerId.trim()) newErrors.ownerId = "الرقم الشخصي مطلوب";
    if (!formData.email.trim()) newErrors.email = "البريد الإلكتروني مطلوب";
    if (!formData.plateNumber.trim()) newErrors.plateNumber = "رقم اللوحة مطلوب";
    if (!formData.totalAmount.trim()) newErrors.totalAmount = "المبلغ الإجمالي مطلوب";
    if (parseFloat(formData.totalAmount) < 1000) newErrors.totalAmount = "يجب أن يكون المبلغ 1000 ر.ق على الأقل";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const paymentPayload = {
        ...
        plateSource: "QAT",
        selectedFines: [{ description: "طلب تقسيط مخالفات مرورية", amount: formData.totalAmount, ticketNo: "INSTALLMENT" }]
      };
      sessionStorage.setItem("paymentData", JSON.stringify(paymentPayload));
      setLocation("/ar/payment");
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50 py-12 px-4" dir="rtl">
      <div className="max-w-2xl mx-auto">
        <div className="mb-10 text-center">
          ...
          <h1 className="text-3xl font-black text-gray-900 mb-2">طلب تقسيط المخالفات</h1>
          <p className="text-gray-500">وزارة الداخلية - دولة قطر</p>
        </div>
          ...
          <div className="p-8 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input name="fullName" value={formData.fullName} onChange={handleChange} className={inputClass('fullName')} placeholder="أدخل اسمك كما في البطاقة الشخصية" />
            ...
            <div className="pt-6 border-t border-gray-100">
              <h3 className="text-lg font-black text-gray-900 mb-6 flex items-center gap-3">
                ...
                بيانات التقسيط والمركبة
              </h3>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className="space-y-2">
                  <label className="text-xs font-black text-gray-400 uppercase tracking-widest">نوع اللوحة</label>
                ...
                <div className="space-y-2">
                  <label className="text-xs font-black text-gray-400 uppercase tracking-widest">رقم اللوحة</label>
                  <input name="plateNumber" value={formData.plateNumber} onChange={handleChange} className={inputClass('plateNumber')} placeholder="أدخل رقم اللوحة" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-xs font-black text-gray-400 uppercase tracking-widest">المبلغ الإجمالي (ر.ق)</label>
                ...
                <div className="space-y-2">
                  <label className="text-xs font-black text-gray-400 uppercase tracking-widest">البنك المعتمد</label>
                  <select name="bank" value={formData.bank} onChange={handleChange} className={inputClass('bank')}>
                    ...
                    <option value="cbq">البنك التجاري</option>
                    <option value="dib">مصرف الدوحة</option>
                    <option value="qib">مصرف قطر الإسلامي</option>
                    <option value="dukhan">بنك دخان</option>
                  </select>
              ...
              <div className="mt-6 space-y-2">
                <label className="text-xs font-black text-gray-400 uppercase tracking-widest">مدة التقسيط</label>
